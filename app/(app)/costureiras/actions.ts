...
export async function updateSeamstress(_prev: SeamstressActionState, formData: FormData): Promise<SeamstressActionState> {
  ...
  if (!user) return { error: 'Faça login para editar costureiras.' };

  const { error } = await supabase.from('seamstresses').update({
    name, role, phone, pix, address, specialty, notes, active,
  }).eq('id', id);
